...
from benchmark.hpa.hpa.problem import *


class BotorchHPA(BaseTestProblem):
    def __init__(self, problem_name, n_div=4, level=1, NORMALIZED=True, noise_std=None, negate=True):
...
def DefineProblems(problem, dim=0, dim_emb=0, noise_std=None, negate=True, n_div=4):
    if 'HPA' in problem:
        ...
        if dim > 0:
            try:
                fun = eval(problem+'(dim='+str(dim)+', noise_std='+str(noise_std)+', negate='+str(negate)+')')
            except:
                fun = eval(problem+'(noise_std='+str(noise_std)+', negate='+str(negate)+')')
            return fun
        else:
            return eval(problem+'(noise_std='+str(noise_std)+', negate='+str(negate)+')')
